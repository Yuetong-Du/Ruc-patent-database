...
def dashboard():
    ...
    result_middle2 = (db.session.query(GApplication.application_year,
                                       func.count(GApplication.application_year).label('number'))
                      .group_by(GApplication.application_year)
                      .order_by(GApplication.application_year.asc()).all())
...
def patent_detail(patent_id):
    ...
    alert1 = InventorAlert.query.filter_by(patent_number=patent_id).first()
    alert2 = AppliDelay.query.filter_by(patent_number=patent_id).first()
    alert3 = BeyondInventor.query.filter_by(patent_number=patent_id).first()
    ...
    return render_template('patent_detail.html', 
                           ...
                           assignee=assignee,
                           location=location,
                           InventorAlert=alert1,
                           AppliDelay=alert2,
                           BeyondInventor=alert3)
...
def applicant_apply():
    ...
    if form.validate_on_submit():
        ...
        patent_application.assignee = form.assignee.data
        patent_application.country = form.country.data
        patent_application.city = form.city.data
        patent_application.county = form.county.data
        patent_application.state = form.state.data
...
def patent_search():
    ...
    if form.validate_on_submit():
        ...
        session['search_params'] = {
            ...
            'claims_least': form.claims_least.data,
        }
